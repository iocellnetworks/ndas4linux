...
static int ndc_register(int argc, char *argv[]) {
    ...
    while ((c = getopt_long(argc, argv, "n:b", long_options, &option_index)) != EOF) 
    {
        if ( c == '?' ) {
            cmd_usage(argv[0]);
            ret = -1;    
...
{
	/* 
	 * since this ndas is no longer backward compatible and it seems like
	 * 1 was the positive reply, we will just make this return 1 for now. 
	 */
	return 1;
}

static int set_io_scheduler(char* ndas_device) {
   ...
   strcpy(first, "/sys/block/");
   strcpy(middle, ndas_device);
   strcpy(last, "/queue/scheduler");
   strcpy(full_path, first);
   strcat(full_path, middle);
   strcat(full_path, last);

	/* open the scheduler file */
   if((file = fopen(full_path,"w"))==NULL) {
     return -1; 
   } else {
     dbg_ndadmin(1, "set scheduler 'noop' on %s\n", full_path);
	  fprintf(file,"%s","noop"); /* change the setting */
	  fclose(file); /* done! */
	  return 0;
	}
}
...
static int ndc_enable(int argc, char *argv[]) 
{
    ...
    if ( ioctl(ndas_fd,IOCTL_NDCMD_ENABLE,&arg) != 0 ) {
        ...
        case NDAS_ERROR_TOO_MANY_INVALID:
            fprintf(stderr,"%s: Too many defective disks."
                " At least %d disk(s) should be valid to enable."
...
#ifdef NDAS_DONT_CARE_SCHEDULER
			ret = set_io_scheduler(devname);
			if (ret < 0) {
			    printf("could not set the scheduler for %s \n", devname);
			}
#endif	
    ...
    printf("Slot %d may be a member of RAID disk.\n"
        "To enable the raid, execute the following command\n"
