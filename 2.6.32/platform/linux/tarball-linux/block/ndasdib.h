...
typedef __u64 NDAS_BLOCK_LOCATION;
...
typedef __u8 NDAS_DIB_DISK_TYPE;
...
typedef __u8 NDAS_DIBEXT_DISK_TYPE;
...
static const __s32 NDAS_DIB_SIGNATURE = 0xFE037A4E;
static const __u8 NDAS_DIB_VERSION_MAJOR = 0;
static const __u8 NDAS_DIB_VERSION_MINOR = 1;

typedef struct _NDAS_DIB {

    __u32    Signature;        // 4 (4)
    
    __u8    MajorVersion;    // 1 (5)
    __u8    MinorVersion;    // 1 (6)
    __u8    reserved1[2];    // 1 * 2 (8)

    __u32    Sequence;        // 4 (12) ignore

    __u8    EtherAddress[6];    // 1 * 6 (18) mine if not ignore
    __u8    UnitNumber;        // 1 (19)
    __u8    reserved2;        // 1 (20)

    __u8    DiskType;        // 1 (21) 
    __u8    PeerAddress[6];    // 1 * 6 (27)
    __u8    PeerUnitNumber; // 1 (28)
    __u8    reserved3;        // 1 (29)

    __u8    UsageType;        // 1 (30) ignore
    __u8    reserved4[3];    // 1 * 3 (33)

    unsigned char reserved5[512 - 37]; // should be 512 - ( 33 + 4 )

    __s32    Checksum;        // 4 (512) ignore
...
static const __u8 NDAS_DIB_V2_SIGNATURE[8] = {0x9f, 0x5f, 0x49, 0xf4, 0x9f, 0x4a, 0x40, 0x3f};

//static const __u64 NDAS_DIB_V2_SIGNATURE = 0x3F404A9FF4495F9F;
//#define    DISK_INFORMATION_SIGNATURE_V2    0x3F404A9FF4495F9F
static const __u32 NDAS_DIB_VERSION_MAJOR_V2 = 1;
static const __u32 NDAS_DIB_VERSION_MINOR_V2 = 1;
...
typedef struct _NDAS_DIB_V2 {
    union {
        struct  {
            ...
            __u64    sizeXArea; // in sectors, always 2 * 1024 * 2 (2 MB)

            __u64    sizeUserSpace; // in sectors

            __u32    iSectorsPerBit; // dirty bitmap . default : 128
            __u32    iMediaType; // NDAS Media Type
            __u32    nDiskCount; // 1 ~ . physical disk units
            __u32    iSequence; // 0 ~
            __u8    AutoRecover; // Recover RAID automatically
            __u8    Reserved0[3];
            __u64    AodSblockLoc;
            __u32    AodSblockLen;
...
typedef struct _NDAS_CONTENT_ENCRYPT_BLOCK {
    union {
        struct {
            __u64    Signature;    // Little Endian
            // INVARIANT
            __u32 Revision;   // Little Endian
            // VARIANT BY REVISION (MAJOR)
            __u32    Method;        // Little Endian
            __u32    Reserved_0;    // Little Endian
            __u32    KeyLength;    // Little Endian
            __u32    Reserved_1;    // Little Endian
            union {
                __u8    Key32[32 /8];
                __u8    Key128[128 /8];
                __u8    Key192[192 /8];
                __u8    Key256[256 /8];
                __u8    Key[NDAS_CONTENT_ENCRYPT_MAX_KEY_LENGTH];
            } __attribute__((packed)) u;
            __u8 Fingerprint[16]; // 128 bit, KeyPairMD5 = md5(ks . kd)
        } __attribute__((packed)) s;
        unsigned char bytes_508[508];
    }__attribute__((packed)) u;
    __u32 CRC32;        // Little Endian
...
typedef struct _LAST_WRITTEN_SECTOR {
    __u64    logicalBlockAddress;
    __u32    transferBlocks;
    __u32    timeStamp;
...
typedef struct _NDAS_UNIT_META_DATA {
    __u16    iUnitDeviceIdx; // Index in NDAS_DIB_V2.UnitDisks
    __u8    UnitDeviceStatus; // NDAS_UNIT_META_BIND_STATUS_*
    __u8    Reserved[5];
...
typedef struct _NDAS_RAID_META_DATA {
    union {
        struct {
            __u64    magic;    // Little Endian
            __u32 usn; // Universal sequence number increases 1 each writes.
            __u32 state;
        } __attribute__((packed)) s;
        __u8 bytes_248[248];
    } __attribute__((packed)) u; // 248
