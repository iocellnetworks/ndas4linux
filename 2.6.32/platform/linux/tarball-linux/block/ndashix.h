...
typedef struct _NDAS_HIX_HEADER {
    __u32 signature;
    __u8 revision;                // HIX_CURRENT_REVISION
    ...
    unsigned int reply_flag : 1;        // 0 for request, 1 for reply
#endif    
    __u16 length;                // Including header
    __u8 hostid[16];        // Generally 16 byte GUID is used
    __u8 reserved[8];            // Should be zero's
...
typedef struct _NDAS_HIX_HOST_INFO_AD_HOC {
    bool unicode;
    __u8 FieldLength; // up to 255 chars
    __u8 DataLength; // up to 255 chars
    //__u8 Field[];
    //__u8 Data[];
...
// Required
// Field: __u8, one of HIX_HIC_OS_xxxx

typedef __u8 HIX_HIC_OS_FAMILY_TYPE;
...
// Optional
// Field: __u8 Count, __u8 AddressLen = 6, __u8[][6]
#define HIX_HIC_ADDR_LPX                0x00000100

// Optional
// Field: __u8 Count, __u8 AddressLen = 4, __u8[][4]
#define HIX_HIC_ADDR_IPV4                0x00000200

// Optional
// Field: __u8 Count, __u8 AddressLen = 16, __u8[][16]
...
// Required
// Field: __u32, flags (network byte order)
...
typedef __u32 HIX_HIC_HOST_FEATURE_TYPE;

// Required
// Field: __u8, one or more of HIX_HIC_TRANSPORT_{LPX,IP}
#define HIX_HIC_TRANSPORT                0x00004000

#define HIX_TF_LPX        0x01
#define HIX_TF_IP        0x02

typedef __u8 HIX_HIC_TRANSPORT_TYPE;
...
typedef struct _NDAS_HIX_HOST_INFO_ENTRY {
    __u8 length; // Entire length (e.g. 1 + 4 + sizeof(data))
    __u32 class; // 
    __u8 data[0]; // at least one byte data is required
    // Class Specific Data
...
typedef struct _NDAS_HIX_QUERY_HOST_INFO_REPLY { // sendto
    ...
    struct _NDAS_HIX_QUERY_HOST_INFO_REPLY_DATA {
         ...
         */
        __u8 length;   
        __u32 contains; // Class contains Flags
...
typedef struct _NDAS_HIX_SURRENDER_ACCESS_REQUEST { //sendto
    hix_header header;
    __u8 entry_count; // Should be always 1
    hix_udev_entry udev;
