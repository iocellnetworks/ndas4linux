#ifndef _LINUX_NDASFAT_FS_H
#define _LINUX_NDASFAT_FS_H
...
#include "ndasfat_fs_i.h"
#include "ndasfat_fs_sb.h"
...
C_ASSERT( 0, sizeof(struct ndasfat_sb_info) <= sizeof(((struct super_block *)NULL)->u) );

static inline struct ndasfat_sb_info *MSDOS_SB(struct super_block *sb)
{
	return (struct ndasfat_sb_info *)&sb->u;
}

C_ASSERT( 1, sizeof(struct ndasfat_inode_info) <= sizeof(((struct inode *)NULL)->u) );

static inline struct ndasfat_inode_info *MSDOS_I(struct inode *inode)
{
	return (struct ndasfat_inode_info *)&inode->u;
}
...
#ifdef NDAS_CRYPTO
extern struct inode *fat_build_inode(struct super_block *sb, struct inode *parent,
...
#ifdef NDAS_CRYPTO
extern void fat_dirty_inode(struct inode *inode);
#endif
...
#ifdef NDAS_CRYPTO
extern struct buffer_head *fat_clusters_read(struct super_block *sb);
extern struct buffer_head *fat_bread_direct(struct super_block *sb, int block);
extern void fat_buffer_sync(struct super_block *sb, struct buffer_head *bh);
#endif

#ifdef NDAS_CRYPTO
extern void fat_clusters_flush(struct super_block *sb, struct buffer_head *bh);
#else
...
#ifdef NDAS_CRYPTO
extern int fat_subdirs(struct inode *dir, struct inode *parent);
#else
