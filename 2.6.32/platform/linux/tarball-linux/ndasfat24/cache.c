...
{
	return MSDOS_SB(sb)->ndas_cvf_format->fat_access(sb,nr,new_value);
}

int fat_bmap(struct inode *inode,int sector)
{
	return MSDOS_SB(inode->i_sb)->ndas_cvf_format->cvf_bmap(inode,sector);
}
...
int default_fat_access(struct super_block *sb,int nr,int new_value)
{
	...
	dbg_call( 8, "bh->blocknr = %ld, bh->size = %d\n", bh->b_blocknr, bh->b_size );
...
int fat_free(struct inode *inode,int skip)
{
	...
	if (MSDOS_SB(inode->i_sb)->fat_bits == 32) {
		...
		bh = fat_clusters_read(inode->i_sb);
		if (!bh) {
...
#ifdef NDAS_CRYPTO
	if (MSDOS_SB(inode->i_sb)->fat_bits == 32) {
		fat_clusters_flush(inode->i_sb, bh);
		fat_cluster_unlock(inode->i_sb);
	}
#endif
