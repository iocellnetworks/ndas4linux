...
{
	return MSDOS_SB(sb)->ndas_cvf_format->cvf_bread(sb,block);
}
struct buffer_head *fat_getblk(struct super_block *sb, int block)
{
	return MSDOS_SB(sb)->ndas_cvf_format->cvf_getblk(sb,block);
}
void fat_brelse (struct super_block *sb, struct buffer_head *bh)
{
	if (bh) 
		MSDOS_SB(sb)->ndas_cvf_format->cvf_brelse(sb,bh);
}
void fat_mark_buffer_dirty (
	struct super_block *sb,
	struct buffer_head *bh)
{
	MSDOS_SB(sb)->ndas_cvf_format->cvf_mark_buffer_dirty(sb,bh);
}
void fat_set_uptodate (
	...
	int val)
{
	MSDOS_SB(sb)->ndas_cvf_format->cvf_set_uptodate(sb,bh,val);
}
int fat_is_uptodate(struct super_block *sb, struct buffer_head *bh)
{
	return MSDOS_SB(sb)->ndas_cvf_format->cvf_is_uptodate(sb,bh);
}
...
{
	MSDOS_SB(sb)->ndas_cvf_format->cvf_ll_rw_block(sb,opr,nbreq,bh);
}
