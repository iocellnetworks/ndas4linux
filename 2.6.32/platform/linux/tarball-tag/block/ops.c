...
#endif
{
    ...
    dbgl_blk(2,"dev_t=0x%x", bdev->bd_dev);
#else
    slot = SLOT_I(inode);
    dbgl_blk(2,"slot =0x%x", slot);
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,28))
        ndop_release(bdev->bd_disk, mode);
#else
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,28))
    dbgl_blk(5, "disk=%p",disk);
    dbgl_blk(5, "mode=%x",mode);
    
    if(!disk) {
        printk("Weird, open called with disk = 0\n");
        return -ENODEV;
    }
#else
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,28))
int ndop_ioctl(struct block_device *bdev, fmode_t mode, unsigned int cmd, unsigned long arg)
#else
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,28))
    dbgl_blk(3,"ing bdev=%p, mode=%x, cmd=0x%x, arg=%p", bdev, mode, cmd, (void*) arg);
#else
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,28))
    if(!bdev) {
        printk("ndas: open called with bdev = NULL\n");
        return -EIO;
    }
#else
    if(!inode) {
