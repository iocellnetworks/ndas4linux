...
/*
	Samsung DirecTV (MIPS) compiler caused invaled rotation result for these macros.
	So we use inline functions here.
*/
//#define rotr(x,n)   (((x) >> ((int)(n))) | ((x) << (32 - (int)(n))))
//#define rotl(x,n)   (((x) << ((int)(n))) | ((x) >> (32 - (int)(n))))

static inline xuint32 rotr(xuint32 x, xuint32 n)
{
	xuint32 t1, t2;
	t1 = x >> (int)n;
	t2 = x << (32 - (int)n);
...
static inline xuint32 rotl(xuint32 x, xuint32 n)
{
	xuint32 t1, t2;
	t1 = x << n;
	t2 = x >> (32 - (int)n);
...
#define round(x0,x1,ki)                                        \
    l1  = (rotr(x1, 4) ^ *(((xuint32*)key) + ki + 1));   \
    l0  = (x1 ^ *(((xuint32*)key) + ki));                \
...
#define round(x0,x1,ki)                                                     \
    l1  = (rotr(x1, 4) ^ *(((xuint32*)key) + ki + 1)) & 0x3f3f3f3f;   \
    l0  = (x1 ^ *(((xuint32*)key) + ki)) & 0x3f3f3f3f;                \
    x0 ^= sx_tab[0][byte(l0,0)] | sx_tab[1][byte(l1,0)]                     \
        | sx_tab[2][byte(l0,1)] | sx_tab[3][byte(l1,1)]                     \
        | sx_tab[4][byte(l0,2)] | sx_tab[5][byte(l1,2)]                     \
        | sx_tab[6][byte(l0,3)] | sx_tab[7][byte(l1,3)]
