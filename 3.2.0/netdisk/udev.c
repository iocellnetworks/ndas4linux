/*
    to do: Change udev as unit device not single type logical unit. 
    Need to create single type logical unit device types.
*/
#include "xplatcfg.h"
#include <sal/types.h>
...
*/
struct udev_request_block *tir_alloc(xint8 cmd, ndas_io_request_ptr req) 
{
    ...
    if(req) {
		debug_tir(1, "Caller specified request.\n");
        ...
        tir->req = &tir->int_req;
        debug_tir(1, "Using internal request.\n");
    }
    tir->arg = NULL;
...
ndas_error_t ndas_io(int slot, xint8 cmd, ndas_io_request* req, xbool bound_check)
{
	...
	debug_udev(8, "hunting for clues. called for slot %d", slot);
