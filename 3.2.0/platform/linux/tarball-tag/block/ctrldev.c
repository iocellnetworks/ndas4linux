...
static ssize_t
ndas_ctrldev_write(struct file *filp, const char __user *buf, size_t cnt, 
						loff_t *offp)
{
...
NDAS_CALL 
void ndcmd_dev_changed(const char* serial, const char *name, 
							NDAS_DEV_STATUS online);
...
ndas_error_t slot_enable(int s)
{
		...
		slot->info.mode == NDAS_DISK_MODE_MEDIAJUKE) 
	{
		...
		if (strlen(slot->info.ndas_serial) > 8) {
			...
			strncpy(short_serial, slot->info.ndas_serial + 
					(NDAS_SERIAL_EXTEND_LENGTH - NDAS_SERIAL_SHORT_LENGTH), 8);
		} else {
		...
		dbgl_blk(1, "just set slot->disk->%s, slot->%s", slot->disk->disk_name, 
			slot->devname );

		set_capacity(slot->disk, slot->info.sectors);
		dbgl_blk(1, "just set capacity slot->disk, slot->info.sectors:%llu", 
			slot->info.sectors);
	...
	dbgl_blk(4, "adding disk: slot=%d, first_minor=%d, capacity=%llu", s, 
...
LOCAL void slot_disable(int slot)
{
	...
	if ( sd == NULL ) {
		dbgl_blk(0, "ndas: Could not find slot dev %d at slot_disable!!", slot);
		goto out;
	}
...
LOCAL void slot_destroy(int slot)
{
	struct ndas_slot* sd = NDAS_GET_SLOT_DEV(slot); 
	if (sd == NULL) {
		printk("ndas: slot %d doesn't exist: bug caused in race condition\n", 
				slot);
		return;
	}
...
{
	printk(
		"ndas: %s asks to surrender exclusive write access for slot %d\n", 
		"somebody",
		slot
	);
	return NDAS_OK;
...
NDAS_CALL LOCAL 
void ndcmd_enabled_handler(int slot, ndas_error_t err, void* arg)
{
 ...
 */
NDAS_CALL LOCAL 
void ndcmd_disabled_handler(int s, ndas_error_t err, void* arg)
{
	slot_disable((int)s);
}
			...
			NDAS_DEV_STATUS online)
{
	...
	if (online != NDAS_DEV_STATUS_HEALTHY) {
		/* 
		 * Device is offline.         
		 * Don't disable device yet. 
		 * ndascore may be able to connect to the device again.
	...
	for ( i = 0 ; i < dinfo.nr_unit; i++) 
	{
		...
		/* 
		 * to do:
		 * Implement without ndcmd_enabled_handler, ndcmd_disabled_handler 
		 */
	
		ndas_set_slot_handlers(dinfo.slot[i], ndcmd_enabled_handler, 
				ndcmd_disabled_handler, notify_permission_requested, sd);
		dbgl_blk(7, "call ndas_query_slot(dinfo.slot[%d]:%d, &info", i, 
				dinfo.slot[i]);        
...
ndcmd_enable(int slot, int write_mode)
{
	...
	case 0:
		...
		break;
	default:
		dbgl_blk(1, "Unknown write_mode: %d, enabling with read-only", 
				write_mode);
		err = ndas_enable_slot(slot);
		break;
	}
...
LOCAL inline ndas_error_t ndcmd_probe(char *user_array, int sz, int *count, 
		int convert_to_id)
{
	...
	do {
		...
		for ( i = 0; i < c; i++ ) {
			...
			if (convert_to_id) {
				...
				if (ret == NDAS_OK) {
					result = copy_to_user(user_array + (NDAS_ID_LENGTH+1)*i, 
								id, NDAS_ID_LENGTH+1);
					if ( result != 0 ) {
						ret = NDAS_ERROR_INTERNAL; 
						(*count) = 0;
				...
				result = copy_to_user(user_array + (NDAS_SERIAL_LENGTH+1)*i, 
							data[i].serial, NDAS_SERIAL_LENGTH+1);
				if ( result != 0 ) { 
					ret = NDAS_ERROR_INTERNAL; 
					(*count) = 0;
					break;
...
long ndas_ctrldev_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd) {
		...
		case IOCTL_NDCMD_REGISTER: 
		{
			...
			result = copy_from_user(idserial, rarg->ndas_idserial, 
					NDAS_ID_LENGTH + 1);
			if ( result != 0) { return -EINVAL; }
			result = copy_from_user(key, rarg->ndas_key, NDAS_KEY_LENGTH + 1);
			if ( result != 0) { return -EINVAL; }
			result = copy_from_user(&use_serial, &rarg->use_serial, 
					sizeof(rarg->use_serial));
			if ( result != 0) { return -EINVAL; }
			result = copy_from_user(&discover, &rarg->discover, 
					sizeof(rarg->discover));
			if ( result != 0) { return -EINVAL; }
...
int ndas_ctrldev_init(void)
{
	...
	device_create(ndas_ctrldev_class, NULL,
...
{
	device_destroy(ndas_ctrldev_class, MKDEV(NDAS_CHR_DEV_MAJOR, 
		NDAS_CHR_DEV_MINOR));
