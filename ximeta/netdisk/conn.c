/*
    ...
    These functions should be called in single thread context such as ndiod thread or blockable user function call
*/
#include "xplatcfg.h"
#include <sal/types.h>
        ...
        default:
            {
                ...
                debug_conn(1,"lsp error code %x(R=%x,T=%x,S=%x,F=%x) not converted", status,
...
#ifdef DEBUG
    if (is_set){
        debug_conn(4, "Setting target data: 0x%p", *data);
    }
#endif
...
#ifdef DEBUG
        debug_conn(4, "Got target data: 0x%p", *data);
#endif
    }
    return ret;
}
