...
/*        ndShowAliveList(); */
    ...
    case PS2NETDISK_CMD_PROBE_TEST:
        ...
        if (!NDAS_SUCCESS(sz))
            break;
        data = sal_malloc(sizeof(struct ndas_probed) * sz);
        if ( !data ) break;
        err = ndas_probed_list(data,sz);
        if (NDAS_SUCCESS(err)) {
            int i;
            for (i = 0; i < size; i++) {
...
{
    int res;
    sal_debug_print("netdisk.irx: _start:sal_initing...");    

    sal_init();
    sal_debug_print("done\n");    

    sal_debug_print("netdisk.irx:_start:ndas_initing...");    

    ndas_init(16,50);    
    sal_debug_print("done\n");    

    sal_debug_print("netdisk.irx: creating IRX test threads\n");    
