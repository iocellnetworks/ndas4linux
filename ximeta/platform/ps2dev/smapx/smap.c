/* 
   ...
   $Id: smap.c,v 1.13 2004/05/16 13:57:25 oobles Exp $
*/
...
HandleRXInt(int iIntFlags)
{
    ...
    //Iterate through the BDs until we find an no_pending_dpc BD.
...
RXBDInit(SMap* pSMap)
{
    ...
    for    (iA=0;iA<SMAP_BD_MAX_ENTRY;++iA)
    {
        pSMap->pRXBD[iA].ctrl_stat=SMAP_BD_RX_EMPTY;        //Set no_pending_dpc bit
