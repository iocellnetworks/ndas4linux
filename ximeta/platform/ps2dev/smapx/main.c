...
#define MODNAME "smapx"
IRX_ID(MODNAME, 1, 1);

struct irx_export_table _exp_smapx;
...
static SMAP_LOW_LEVEL_INPUT_HOOK ps2low_level_input_hook = NULL;
...
AddToQueue(PBuf* pBuf)
{
    ...
    if    (iReqCNT==0)
    {
        ...
        //The queue is no_pending_dpc, try to send the packet right away.
    ...
    else if    (iReqCNT<MAX_REQ_CNT)
    {
        ...
        //There queue isn't no_pending_dpc but there is atleast one free entry. Store pBuf last in the queue.
...
{
    //When we receive data, the interrupt-handler will invoke this function, which means we are in an interrupt-context. 
    // Pass on the received data to input hook or ps2ip.

    if (ps2low_level_input_hook && ps2low_level_input_hook(pBuf, &NIF)) {
        /* ps2low_level_input_hook returned TRUE(packet handled) */
...
{
    SMAP_LOW_LEVEL_INPUT_HOOK prev;
//    printf("SMapRegisterInputHook entering\n");
    prev = ps2low_level_input_hook;
    ps2low_level_input_hook = hook;
//    printf("SMapRegisterInputHook exiting\n");
    return prev;
}
...
_start(int iArgC,char** ppcArgV)
{
    ...
    //Initialized ok.
    RegisterLibraryEntries(&_exp_smapx);
    return    0;
}
