...
int	dbg_level_ndasfat = DBG_LEVEL_NDASFAT;
...
static int parse_options(char *options,	struct ndasfat_mount_options *opts)
{
			  ...
			  struct buffer_head **bh, struct msdos_dir_entry **de)
{
	...
	dbg_call(1, "qname->name = %s, qname->len = %d\n", qname->name, qname->len);
...
struct dentry *vfat_lookup(struct inode *dir,struct dentry *dentry)
{
	...
	dbg_call(1, "entered\n");
	dbg_call(1, "name=%s, len=%d\n", dentry->d_name.name, dentry->d_name.len);
#ifdef NDAS_CRYPTO
	fat_cluster_lock(dir->i_sb);
#endif
...
#ifdef NDAS_CRYPTO
	inode = fat_build_inode(dir->i_sb, dir, de, sinfo.i_pos, &res);
#else
...
#ifdef NDAS_CRYPTO
	inode = fat_build_inode(sb, dir, de, sinfo.i_pos, &res);
#else
		...
		struct inode *new_dir,struct dentry *new_dentry)
{
...
#ifdef NDAS_CRYPTO
	if (old_dir->i_ino == MSDOS_ROOT_INO) {
	...
	if (is_dir) {
		...
		if (new_inode) {
			...
			new_dir->i_nlink++;
#ifdef NDAS_CRYPTO
			if (new_dir->i_ino == MSDOS_ROOT_INO) {
...
#ifdef NDAS_CRYPTO
	if (old_dir->i_ino == MSDOS_ROOT_INO) {
		fat_buffer_sync(sb, old_bh);
		fat_buffer_sync(sb, dotdot_bh);
	}

	if (new_dir->i_ino == MSDOS_ROOT_INO) {
		fat_buffer_sync(sb, new_bh);
	}
#endif
				    ...
				    int silent)
{
	...
	dbg_call(1, "entered\n");
#ifdef NDAS_CRYPTO
	dbg_call(1, "initialize super block lock\n");
	INIT_LIST_HEAD(&MSDOS_SB(sb)->fat_free_cluster_list);
	init_MUTEX(&MSDOS_SB(sb)->fat_free_cluster_list_lock);
	MSDOS_SB(sb)->fat_free_cluster_alloc_count = 0 ;
	init_MUTEX(&MSDOS_SB(sb)->cluster_lock);
