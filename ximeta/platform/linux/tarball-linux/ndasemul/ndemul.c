...
#include "gtypes.h" /* g_ntohl g_htonl g_ntohs g_htons */
...
int	dbg_level_nemu = DBG_LEVEL_NEMU;

#define dbg_call(l,x...) do { 								\
    if ( l <= dbg_level_nemu ) {								\
        printk("EM|%d|%s|%d|",l,__FUNCTION__, __LINE__); 	\
        printk(x); 											\
    } 														\
} while(0)
...
#define NEMUL_MAX_BLOCKS	(64*1024/PAGE_SIZE + 2)

ndas_error_t lpx_sendpages(struct socket *sock, sal_page** pages, size_t offset, size_t size, int flags)
{
	ndas_error_t	err;

	struct iovec iov[NEMUL_MAX_BLOCKS];
	size_t iov_num = 0;
    ...
    while (psize > 0) {
        ...
        size_t boffset = poffset % PAGE_SIZE;
        size_t bsize = min_t(size_t, psize, PAGE_SIZE - boffset);
		...
		iov[iov_num].iov_base = page_address(page) + boffset;
		iov[iov_num].iov_len = bsize;
	...
	NDAS_BUG_ON( iov_num == 0 );

	err = lpx_sock_sendmsg( sock, iov, iov_num, size, 0 );

	if (err != size) {

    	dbg_call( 1, "lpx_sock_sendmsg fail err = %x\n", err );
...
ndas_error_t lpx_is_recv_data(struct socket *fd) {
...
#else
static struct socket *ListenSock = NULL;
#endif
...
#else
	struct socket *sockdesc = (struct socket *)read_desc->arg.data;
#endif
	xsize_t	written;
...
#endif
	if(ret < 0) {
		debug_emu(1, "lpx_send failed ret=%d", ret);
		written = 0;
	} else {
...
#else
static int SendIt(struct socket *sock, void *buf, int len, int *res)
#endif
{
	ndas_error_t err;

	err = lpx_sock_send( sock, buf, len, 0 );
...
void ndasemu_aio_complete(struct _lpx_aio * aio, void *userparam)
{
    ...
    //debug_emu(4, "aio=%p sockfd=%p len=%d comp=%d err=%d ret=%d",
...
#else
int SendtoIt(struct socket *lpxfd, void * buff, int len, unsigned flags,
                           struct sockaddr_lpx *addr, int addr_len)
#endif
{
#if 0
	return lpx_sendto(lpxfd, buff, len, flags, addr, addr_len);
#else
	return lpx_sock_sendto(lpxfd, buff, len, flags, addr, addr_len);
#endif
}
...
#else
int RecvIt(struct socket *sock, void *buf, int len, int *res, lpx_aio_head_lookahead aio_head_lookahead, void *userparam)
#endif
{
	ndas_error_t err;
	...
	err = lpx_sock_recv( sock, buf, len, 0 );
	...
	if (aio_head_lookahead) {
		...
		err = aio_head_lookahead( NULL, userparam, buf, len, &request_len_adjustment );
...
ndasemu_send_reply(
	...
	iResult = lpx_sendpages(task->sockfd, task->pdurep_pages, task->pdurep_offset, iTemp, 0);

	if (NDAS_SUCCESS(iResult)) {
	
		ndasemu_aio_complete( NULL, task );
		res = iTemp;
	}
...
)
{
...
#else
		lpx_sock_close(thread_param->sock);
		thread_param->sock = NULL;
#endif
	...
	while (pSessionData->iSessionPhase != LOGOUT_PHASE) {
        ...
        for(;pending_tasks < NDASEMU_MAX_PENDING_TASKS &&
            ...
            //
            debug_emu(1, "fd %p: Opcode %x", pSessionData->connSock, cur_ndastask->pdu_rep->gen.Opcode);
...
#else
		lpx_sock_close(pSessionData->connSock);
		pSessionData->connSock = NULL;
#endif
...
void* GenPnpMessage(void *arg)
{
    ...
    err = lpx_sock_create( LPX_TYPE_DATAGRAM, 0, &pnpsock );

	debug_emu( 1, "pnpsock=%p\n", pnpsock );
    ...
    err = lpx_sock_bind( pnpsock, &slpx, sizeof(slpx) );

   	if (!NDAS_SUCCESS(err)) {
		
       	debug_emu( 1, "bind failed for sock %p:%d\n", pnpsock, err );
		NDAS_BUG_ON(true);
...
void* ListenThreadProc(void* arg)
{
	...
	int AcceptedSock;
#else
	struct socket *AcceptedSock = NULL;
#endif
	xuint32 addr_len;
	...
	while (ListenThreadRunning) {
		...
		//
        debug_emu(1, "Wait for accepting from listen sock %p", ListenSock);
...
#else
		err = lpx_sock_accept(ListenSock, &AcceptedSock, &slpx, &addr_len, 0);
        ...
        debug_emu(1, "Accepted a connection from %02x:%02x:%02x:%02x:%02x:%02x, fd=%p", 
...
ndas_emu_start(
    ...
    ndaserr = lpx_sock_create( LPX_TYPE_STREAM, 0, &ListenSock );
    ...
    ndaserr = lpx_sock_bind( ListenSock, &slpx, sizeof(slpx) );
	...
	ndaserr = lpx_sock_listen( ListenSock, MAX_CONNECTION );
