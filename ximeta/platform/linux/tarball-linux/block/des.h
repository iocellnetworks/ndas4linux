...
static inline __u32 rotr(__u32 x, __u32 n)
{
	__u32 t1, t2;
	t1 = x >> (int)n;
	t2 = x << (32 - (int)n);
...
static inline __u32 rotl(__u32 x, __u32 n)
{
	__u32 t1, t2;
	t1 = x << n;
	t2 = x >> (32 - (int)n);
...
#define round(x0,x1,ki)                                                     \
    l1  = (rotr(x1, 4) ^ *(((__u32*)key) + ki + 1)) & 0x3f3f3f3f;   \
    l0  = (x1 ^ *(((__u32*)key) + ki)) & 0x3f3f3f3f;                \
    x0 ^= sx_tab[0][byte(l0,0)] | sx_tab[1][byte(l1,0)]                     \
        | sx_tab[2][byte(l0,1)] | sx_tab[3][byte(l1,1)]                     \
        | sx_tab[4][byte(l0,2)] | sx_tab[5][byte(l1,2)]                     \
        | sx_tab[6][byte(l0,3)] | sx_tab[7][byte(l1,3)]
