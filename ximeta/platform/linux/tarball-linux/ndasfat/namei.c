...
{
	struct ndasfat_mount_options *opts = &MSDOS_SB(dir->i_sb)->options;
...
static int vfat_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	{	
		...
		memset( &ndas_lock, 0, sizeof(ndas_lock) );

		ndas_lock.lock_type = 1;
		err = ioctl_by_bdev( sb->s_bdev,  IOCTL_NDAS_LOCK, (unsigned long)&ndas_lock );

		dbg_call( 1, "lock err = %d, ndas_lock.lock_status = %d\n", err, ndas_lock.lock_status );

		if (err == 0 && ndas_lock.lock_status == 0) {

			ndas_lock.lock_type = 0;
			err = ioctl_by_bdev( sb->s_bdev,  IOCTL_NDAS_LOCK, (unsigned long)&ndas_lock );	

			dbg_call( 1, "unlock err = %d, ndas_lock.lock_status = %d\n", err, ndas_lock.lock_status );
		}
	}
