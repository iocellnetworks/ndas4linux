...
static int ifoRead_VMG(ifo_handle_t *ifofile) {
  ...
  fprintf(stderr,"Address of audio (%p)\n", &audio[1]);
  fprintf(stderr,"Address of lans_code (%p)\n", &audio[1].lang_code);
  fprintf(stderr,"Address of lang_extension (%p)\n", &audio[1].lang_extension);  
  fprintf(stderr,"Address of code_extension (%p)\n", &audio[1].code_extension);  
  fprintf(stderr,"Address of unknown3 (%p)\n", &audio[1].unknown3);  
  fprintf(stderr,"Address of app_info (%p)\n", &audio[1].app_info);  

  fprintf(stderr,"Address of audio (%p)\n", &audio[2]);
  fprintf(stderr,"Address of lans_code (%p)\n", &audio[2].lang_code);
  fprintf(stderr,"Address of lang_extension (%p)\n", &audio[2].lang_extension);  
  fprintf(stderr,"Address of code_extension (%p)\n", &audio[2].code_extension);  
  fprintf(stderr,"Address of unknown3 (%p)\n", &audio[2].unknown3);  
  fprintf(stderr,"Address of app_info (%p)\n", &audio[2].app_info);  

  fprintf(stderr, "sizeof uint16_t = (%d)\n", sizeof(uint16_t));
  fprintf(stderr, "sizeof vmgi_mat->zero_8 = %d\n", sizeof(vmgi_mat->zero_8));
  fprintf(stderr, "sizeof vmgm_video_attr = %d\n", sizeof(vmgi_mat->vmgm_video_attr));
