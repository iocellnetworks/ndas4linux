#ifndef __MEDIA_JUKE_DISK_H_
#define __MEDIA_JUKE_DISK_H_
...
*		|  4M    |      4M      |
****************************************************************/
#define MS_SECTOR_SIZE_BIT				9
#define MS_SECTOR_SIZE				(1<<MS_SECTOR_SIZE_BIT)

// cluster size 2^28 (256M)
#define MEDIA_CLUSTER_SIZE_BIT			28
#define MEDIA_CLUSTER_SIZE			(1<< MEDIA_CLUSTER_SIZE_BIT)
#define MEDIA_CLUSTER_SIZE_SECTOR_COUNT		(1 << (MEDIA_CLUSTER_SIZE_BIT - MS_SECTOR_SIZE_BIT))
...
#define MEDIA_DISC_INFO_START_ADDR_SECTOR	(1 << (MEDIA_DISC_INFO_START_ADDR_BIT - MS_SECTOR_SIZE_BIT))
...
#define MEDIA_DISC_DATA_START_SECTOR		(1 << ( MEDIA_DISC_DATA_START_BIT - MS_SECTOR_SIZE_BIT))
...
#define MEDIA_DISC_INFO_SIZE_SECTOR		(1 << (  MEDIA_DISC_INFO_SIZE_BIT - MS_SECTOR_SIZE_BIT ))
...
#define MEDIA_META_INFO_SECTOR_COUNT		(1 << (MEDIA_META_INFO_SIZE_BIT - MS_SECTOR_SIZE_BIT))

// ximeta DIV size 2^21 (2M)
#define XIMETA_MEDIA_INFO_SIZE_BIT		21
#define XIMETA_MEDIA_INFO_SIZE			(1 << XIMETA_MEDIA_INFO_SIZE_BIT)
#define XIMETA_MEDIA_INFO_SECTOR_COUNT		(1 << (XIMETA_MEDIA_INFO_SIZE_BIT - MS_SECTOR_SIZE_BIT) )
...
#define DISK_META_START				(1 << (DISK_META_START_BIT - MS_SECTOR_SIZE_BIT))

//	2M
#define DISK_LOG_START_BIT			21
#define DISK_LOG_START 				(1<< (DISK_LOG_START_BIT - MS_SECTOR_SIZE_BIT))
...
#define DISK_META_MIRROR_START			(1<< (DISK_META_MIRROR_START_BIT - MS_SECTOR_SIZE_BIT))

//	5M
#define DISK_LOG_MIRROR_START			( DISK_META_MIRROR_START + (1<<(20-MS_SECTOR_SIZE_BIT)) )
...
typedef unsigned char * PBITMAP;


typedef struct  _ON_DISK_FREE
{
	BITMAP	FreeCluster_bitmap[512];
}
__attribute__((packed))
...
{
	BITMAP	DirtyCluster_bitmap[512];
}
__attribute__((packed))
...
typedef struct  _ON_DISK_META
{
	...
	BITMAP		DiscInfo_bitmap[256];			// offset 256
	BITMAP		FreeCluster_bitmap[512];		// offset 512
	BITMAP		DirtyCluster_bitmap[512];		// offset 1024
}
__attribute__((packed))
ON_DISK_META, *PON_DISK_META;
...
typedef struct _ON_DISK_LOG
{
	...
	unsigned long long time;				// offset 16
...
#define DISC_META_SECTOR_COUNT			(1 << (DISC_META_SIZE_BIT - MS_SECTOR_SIZE_BIT))		
#define DISC_META_HEAD_SECTOR_COUNT		1

#define DISC_META_MIRROR_START_SHIFT_BIT	22
#define DISC_META_MIRROR_SHIFT_SECTOR_BIT	( DISC_META_MIRROR_START_SHIFT_BIT - MS_SECTOR_SIZE_BIT )
...
typedef struct _ON_DISC_ENTRY {
	...
	unsigned long long	time;						// offset 28
...
#define MAX_META_BUFFER				(8*MS_SECTOR_SIZE)
 
#endif //#ifdef XPLAT_JUKEBOX
#endif // #ifndef MEDIA_JUKE_DISK_H
