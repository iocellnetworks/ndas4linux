...
ndas_error_t udev_write_dib(udev_t *udev,NDAS_DIB_V2 *pDIB_V2)
{
    ...
    if (!NDAS_SUCCESS(err)) {
        debug_udev(1, "conn_read_dib failed: %d ioreq.num_sec=%d", err, ioreq.num_sec);
        return err;
    }
...
ndas_error_t ndas_io_atapi_process(logunit_t * l_udev, ndas_io_request * req){
	...
	switch(pc_req->cmd[0])
	{
		...
		case SCSIOP_REPORT_LUNS:
		{
			...
			debug_udev(1, "lunsize is set to %x but not used in this function.", lunsize);
...
ndas_error_t ndas_io_ata_process(logunit_t * l_udev, ndas_io_request * req)
{
	...
	switch(pc_req->cmd[0]){
		...
		case SCSIOP_MODE_SELECT:
		{
			...
			debug_udev(1, "poor parameterlen. It is not used elswhere in this function. So it = %d forever.", parameterlen);
			...
			debug_udev(8, "modeH is not used anywhere in 2.6.43? it is now=%s", (xuint8 *) modeH + sizeof(MODE_PARAMETER_BLOCK) );
