...
static struct nbio_linux* nbio_alloc_splited(struct request *req)
{
    ...
    struct bio_vec* bvec;
    int slot = (long) req->rq_disk->private_data;
...
_end_io(ndas_error_t err, struct request *req, size_t nr_sectors) 
{
    ...
    if ( !uptodate ) {
#endif
        printk("ndas: IO error occurred at sector %d(slot %ld): %s\n", 
#if ( LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,31) )
            (int)blk_rq_pos(req),
#else
            (int)req->sector,
...
//    _print_request(req);
    ...
    if (unlikely( !nbio )) {
        printk("ndas: Out of memory at sector %d(slot %ld): %s\n", 
#if ( LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,31) )
            (int)blk_rq_pos(req),
#else
            (int)req->sector,
    ...
    if (unlikely(!sd->enabled)) {
        ...
        printk("ndas: Not connected %d(slot %ld): %s\n", 
#if ( LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,31) )
...
void nblk_request_proc(struct request_queue *q)
{
    ...
    {
        if (test_bit(NDAS_FLAG_QUEUE_SUSPENDED, &(NDAS_GET_SLOT_DEV((long)SLOT_R(req))->queue_flags)))  {
