    ...
    sal_thread_func f, void *arg)
{
    ...
    pid_t ret = 0;
    struct task_struct *ndas_thread;
    
    ndas_thread = kthread_create((int (*)(void*))f, (void *)arg, name, flags);
    
    if (ndas_thread) {
    		wake_up_process(ndas_thread);
	    if (tid)
	        *tid = (sal_thread_id)(long)ndas_thread;
    		
    } else {
        ret = (long) ndas_thread;
        switch (ret) {
        case ENOMEM:
...
void sal_thread_daemonize(const char* name, ...)
{
    ...
    va_end(args);
/* needed only when user space called kernel_thread */
