...
ndas_error_t slot_enable(int s)
{
        ...
        slot->info.mode == NDAS_DISK_MODE_MEDIAJUKE) 
    {
	    ...
	    dbgl_blk(1, "just set capacity slot->disk, slot->info.sectors:%lu", slot->info.sectors);
    ...
    dbgl_blk(4, "adding disk: slot=%d, first_minor=%d, capacity=%lu", s, slot->disk->first_minor, slot->info.sectors);
    add_disk(slot->disk);
