...
LSP_INLINE 
lsp_call
lspp_get_aux_buffer(lsp_handle_context_t* context)
{
	return (lsp_uint8_t*) context->session_aux_data_buffer;
}
...
#define lspp_get_aux_buffer(context) \
	((lsp_uint8_t*)(context->session_aux_data_buffer))
#endif
...
#define LSP_SESSION_AUX_BUFFER_SIZE		4096
...
#define LSP_SESSION_BUFFER_SIZE_MACRO (\
	...
	LSP_SESSION_AUX_BUFFER_SIZE)

const lsp_uint32_t LSP_SESSION_BUFFER_SIZE = LSP_SESSION_BUFFER_SIZE_MACRO;

LSP_C_ASSERT(lsp_bufsize, LSP_SESSION_BUFFER_SIZE_MACRO <= 6144);
	...
	__in lsp_uint32_t session_buffer_size)
{
	...
	buf += LSP_SESSION_AUX_BUFFER_SIZE;
	context->session_aux_data_buffer = buf;
	return context;
}
...
void *
lsp_call 
lsp_get_pkt_aux_buffer(lsp_handle_context_t* context)
{
	return (void *)lspp_get_aux_buffer(context);
}
	...
	__in struct _lsp_request_packet_t* request)
{
	...
	{
		if (h->session.hardware_data.hardware_version >= LSP_HARDWARE_VERSION_1_1 &&
			LSP_IDE_CMD_IDENTIFY_PACKET_DEVICE != request->u.ide_command.
...
{
	lsp_request_packet_t* request = NULL;
	request = &h->session.internal_packets[0];
	lsp_memset(request, 0, sizeof(lsp_request_packet_t));
	lsp_build_ide_set_features(
		request,
		h,
		subcommand_code,
		subcommand_specific_0,
		subcommand_specific_1,
		subcommand_specific_2,
		subcommand_specific_3
		);
