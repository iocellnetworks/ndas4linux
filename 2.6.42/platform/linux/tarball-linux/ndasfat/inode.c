...
#else
static inline int __fat_get_blocks(struct inode *inode, sector_t iblock,
				  unsigned long *max_blocks,
				  struct buffer_head *bh_result, int create)
#endif
{
...
{
	struct ndasfat_sb_info *sbi = MSDOS_SB(inode->i_sb);
	spin_lock(&sbi->inode_hash_lock);
...
#endif
{
	struct ndasfat_inode_info *ei = (struct ndasfat_inode_info *)foo;
...
static int fat_init_inodecache(void)
{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,23))
	fat_inode_cachep = kmem_cache_create("fat_inode_cache",
					     sizeof(struct ndasfat_inode_info),
...
{
	struct ndasfat_sb_info *sbi = MSDOS_SB(dentry->d_sb);
...
static int fat_show_options(struct seq_file *m, struct vfsmount *mnt)
{
	struct ndasfat_sb_info *sbi = MSDOS_SB(mnt->mnt_sb);
	struct ndasfat_mount_options *opts = &sbi->options;
...
#endif
{
	...
	sbi = kzalloc(sizeof(struct ndasfat_sb_info), GFP_KERNEL);
	if (!sbi)
		return -ENOMEM;
	sb->s_fs_info = sbi;
