...
int fat_readdir(struct file *filp, void *dirent, filldir_t filldir)
{
	...
	fat_cluster_lock(inode->i_sb);
	res = fat_readdirx(inode, filp, dirent, filldir, 0, 0);
	fat_cluster_unlock(inode->i_sb);
		  ...
		  unsigned int cmd, unsigned long arg)
{
	...
	default:
		/* forward ioctl to CVF extension */
	       if (MSDOS_SB(inode->i_sb)->ndas_cvf_format &&
		   MSDOS_SB(inode->i_sb)->ndas_cvf_format->cvf_dir_ioctl)
		       return MSDOS_SB(inode->i_sb)->ndas_cvf_format
		  ...
		  struct msdos_dir_entry **de, loff_t *i_pos)
{
	...
	dbg_call(1, "ino = %ld, slots = %d, i_pos = %ld\n", dir->i_ino, slots, (long)*i_pos);
	offset = curr = 0;
	*bh = NULL;
	row = 0;
	while (fat_get_entry(dir,&curr,bh,de,i_pos) > -1) {
//		printk("[%d,%d,%ld,%ld]%s\n", row, slots, (long)offset, (long)curr, (*de)->name);
		if (IS_FREE((*de)->name)) {
			if (++row == slots)
				return offset;
		} else {
...
#ifdef NDAS_CRYPTO
	if (parent->i_ino == MSDOS_ROOT_INO) {
		fat_buffer_sync(sb, bh);
	}
#endif
	fat_brelse(sb, bh);
