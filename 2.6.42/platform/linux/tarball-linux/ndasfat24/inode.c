...
extern struct ndas_cvf_format default_cvf;
...
struct inode *fat_iget(struct super_block *sb, loff_t i_pos)
{
	...
	struct ndasfat_inode_info *i;
	struct inode *inode = NULL;

	spin_lock(&fat_inode_lock);
	list_for_each(walk, p) {
		i = list_entry(walk, struct ndasfat_inode_info, i_fat_hash);
...
#ifdef NDAS_CRYPTO
static void fat_fill_inode(struct inode *inode, struct inode *parent, struct msdos_dir_entry *de);
struct inode *fat_build_inode(struct super_block *sb, struct inode *parent,
...
#endif
{
	...
	*res = 0;
	...
	dbg_call(1, "inode->i_ino = %ld\n", inode->i_ino);

#ifdef NDAS_CRYPTO
	fat_fill_inode(inode, parent, de);
#else
...
void fat_put_super(struct super_block *sb)
{
	...
	fat_cluster_lock(sb);
#endif
	if (MSDOS_SB(sb)->ndas_cvf_format->cvf_version) {
		dec_ndas_cvf_format_use_count_by_version(MSDOS_SB(sb)->ndas_cvf_format->cvf_version);
		MSDOS_SB(sb)->ndas_cvf_format->unmount_cvf(sb);
	}
#ifdef NDAS_CRYPTO
	{
		...
		if (MSDOS_SB(sb)->fat_bits == 32) {
			...
			if( !bh ) {
				unlock_fat(sb);
				fat_cluster_unlock(sb);
				printk("fat_put_user: Can't read fsinfo\n");
				return;
			}
		}
		fat_deallocate_free_cluster_list(sb);
		if (MSDOS_SB(sb)->fat_bits == 32) {
...
static int parse_options(char *options,int *fat, int *debug,
			 struct ndasfat_mount_options *opts,
			 char *ndas_cvf_format, char *cvf_options)
{
	...
	*debug = *fat = 0;
	...
	for (this_char = strtok(options,","); this_char;
		...
		else if (!strcmp(this_char,"ndas_cvf_format")) {
			if (!value)
				return 0;
			strncpy(ndas_cvf_format,value,20);
		}
...
#ifdef NDAS_CRYPTO
	inode->i_nlink = fat_subdirs(inode, NULL)+2;
#else
		...
		struct inode_operations *fs_dir_inode_ops)
{
	...
	char buf[50];
	int i;
	char ndas_cvf_format[21];
	char cvf_options[101];

	ndas_cvf_format[0] = '\0';
	cvf_options[0] = '\0';
	sbi->ndas_cvf_format = NULL;
	...
	if (!parse_options((char *) data, &fat, &debug, &opts,
			   ndas_cvf_format, cvf_options))
		goto out_fail;
	/* N.B. we should parse directly into the sb structure */
	memcpy(&(sbi->options), &opts, sizeof(struct ndasfat_mount_options));
 ...
 */
	...
	if (!b->fat_length && b->fat32_length) {
		...
		if (fsinfo_block != 0) {
#ifdef NDAS_CRYPTO
			fsinfo_bh = fat_bread_direct(sb, fsinfo_block);
#else
	...
	sbi->ndas_cvf_format = &default_cvf;
	if (!strcmp(ndas_cvf_format, "none"))
		i = -1;
	else
		i = detect_cvf(sb,ndas_cvf_format);
	if (i >= 0)
		error = ndas_cvf_formats[i]->mount_cvf(sb, cvf_options);
	if (error || debug) {
	...
	if (!sb->s_root)
		goto out_no_root;
	if(i >= 0) {
		sbi->ndas_cvf_format = ndas_cvf_formats[i];
...
int fat_statfs(struct super_block *sb,struct statfs *buf)
{
	...
	if (MSDOS_SB(sb)->ndas_cvf_format &&
	    MSDOS_SB(sb)->ndas_cvf_format->cvf_statfs)
		return MSDOS_SB(sb)->ndas_cvf_format->cvf_statfs(sb,buf,
...
#endif
{
	...
	dbg_call(1, "inode->i_ino = %ld, inode->i_start = %d, de->attr = %x, de->name = %s, de->start = %d, de->starthi = %d\n", inode->i_ino, MSDOS_I(inode)->i_start, de->attr, de->name, CF_LE_W(de->start), CF_LE_W(de->starthi));
...
#ifdef NDAS_CRYPTO
		inode->i_nlink = fat_subdirs(inode, parent);
#else
...
#ifdef NDAS_CRYPTO
void fat_dirty_inode(struct inode *inode)
{
	struct super_block *sb = inode->i_sb;
	struct buffer_head *bh;
	struct msdos_dir_entry *raw_entry;
	loff_t i_pos;
	struct dentry *dentry = NULL, *dparent = NULL;

	dbg_call(1, "entered, ino = %ld, pos = %ld\n", inode->i_ino, (long)MSDOS_I(inode)->i_pos);
retry:
	...
	if (dparent && dparent->d_inode && dparent->d_inode->i_ino == MSDOS_ROOT_INO) {
		dbg_call(1, "root entry\n");
		if (!(bh = fat_bread_direct(sb, i_pos >> MSDOS_SB(sb)->dir_per_block_bits))) 
		{
			printk("dev = %s, i_pos = %llu\n", kdevname(inode->i_dev), i_pos);
			fat_fs_panic(sb, "msdos_dirty_inode: unable to read i-node block");
			unlock_kernel();
			return;
		}
	}
